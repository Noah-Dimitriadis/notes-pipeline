...
- [ ] Tailscale installed and joined to the tailnet:
      ...
      `tailscale up`.
  - [ ] Record the box's Tailscale IPv4 (`tailscale ip -4`) — goes in
...
- [ ] Confirm SSH is reachable **only** over Tailscale, not the LAN/WAN —
      ...
      before this project: the box is about to have friends' Anthropic
...
- [ ] Clone/pull this repo onto the box.
...
it's set up):
- [ ] The same `:8000/healthz` and `:3000` URLs, against the box's
